...
<!-- Generated from .claude\commands\pp\doctor.md. Edit the .claude source file and rerun node scripts/sync-copilot-assets.mjs. -->
...
- DB reachable: ✓ / ✗
- CLI versions (cross-check each `null` entry against `cli_probe_timeouts`
  before rendering — a name in that array timed out against
  `PP_DOCTOR_PROBE_TIMEOUT_MS`, it was NOT confirmed missing; render it as
  "probe timed out (raise PP_DOCTOR_PROBE_TIMEOUT_MS)", not "missing"):
  - codex: <version> or "missing" or "probe timed out (raise PP_DOCTOR_PROBE_TIMEOUT_MS)"
  - agy: <version> or "missing" or "probe timed out (raise PP_DOCTOR_PROBE_TIMEOUT_MS)"
  - git: <version>
  - node: <version>
- Vendors configured:
...
1. Call `mcp__pp_harness__get_copilot_claude_tier_models` to get the canonical `{ opus, sonnet, haiku }` map. Print all three so the user can confirm they match the model ids served by their local Claude Code installation.
2. Walk `.github/agents/*.agent.md`. For each agent file (excluding `judge-cross-vendor.agent.md` and `judge-same-vendor.agent.md` which are exempt by design), parse the frontmatter and confirm a `model:` line is present.
