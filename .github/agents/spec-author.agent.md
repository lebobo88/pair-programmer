...
- `primary_producer` — `claude` (default), `codex`, or `agy`
...
3. If `primary_producer != claude`, hand the prompt to the chosen sub-CLI tool (`pp_codex.generate` or `pp_agy.generate`) and use its output. If it returns errors, fall back to writing the artifact directly.
