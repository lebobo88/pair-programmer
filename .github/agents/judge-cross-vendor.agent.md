...
- `artifact_path` — the archived artifact's PROJECT-RELATIVE path (`.harness/<run_id>/<relative_path>`). Findings about the artifact under judgment MUST cite THIS path in `findings_provenance.file`.
...
4.5. **Findings provenance check (R3-tail post-mortem Fix 1.4, 2026-05-21).**
   ...
   - **`quoted_text`** must be a verbatim substring (≥ 8 chars) of the cited file at the cited line. The daemon loads `<project_path>/<file>` — the RUN's target repo root, NOT `<cwd>` and NOT the stage's candidate worktree. Cite paths relative to the project root, and use `artifact_path` for the artifact under judgment.
