...
<!-- Generated from .claude\agents\agents-md-author.md. Edit the .claude source file and rerun node scripts/sync-copilot-assets.mjs. -->
...
- **No file-system fallback.** If `apply_agents_md_patch` fails, do NOT use `read`/`edit` to hand-patch AGENTS.md. The daemon records prev/new sha in the `agents_md_patches` ledger; a direct edit will leave disk and ledger inconsistent and the next run will overwrite your edit.
