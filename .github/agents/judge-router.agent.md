...
<!-- Generated from .claude\agents\judge-router.md. Edit the .claude source file and rerun node scripts/sync-copilot-assets.mjs. -->
...
1. Call `mcp__pp_harness__gate_eligible_judges` with the inputs, including `generator_model` when the parent knows it and `rubric_hint` when the parent has an explicit stage rubric. If `generator_model` is omitted, the daemon will infer Codex/agy defaults where possible.
2. Read the response:
