...
6. **Stage loop.** For each `stage` in `team.stages`, in order:
   ...
   - On `fail/revise`: invoke `reflexion-coach` once. **Escalate the Claude tier by one step** (`retry_tier = shiftTier(initial_tier, +1)`; haiku→sonnet, sonnet→opus, opus stays). cli_floor still applies on retry; cli_cap does NOT (escalation is intentional). Pass `initial_tier`, `retry_tier`, and `model: CLAUDE_TIER_MODELS[retry_tier]` to the coach + re-dispatched generator. Append the escalation to `tier_decisions.json`. If still failing, `finalize_stage(surfaced)` and BREAK.
