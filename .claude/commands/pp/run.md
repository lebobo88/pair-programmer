...
This driver mirrors `.claude/agents/*.md` frontmatter `model:` values so the tier resolver can log the chosen tier even when the Task() call would have fallen through to the agent default. The source of truth is the agent file; this table is for traceability. On a mismatch, the agent file wins — `/pp:doctor` reports the drift.
...
6. **Stage loop.** Pick the stage set by triage class:
   ...
   - On `outcome="pass"`: call `mcp__pp_harness__get_stage_finalize_readiness(stage_id)` **before** any `finalize_stage(status="passed")`.
     ...
     - If readiness returns `next_action="pass_planning_stage"` (X1b plan-first structural gate — the run's taxonomy scope is `standard`/`major` and no earlier stage with kind `spec`/`repro`/`invariants`/`one_pager`/`gdd`/`mechanic_spec`/`tech_design_doc` has finalized `passed` with `finished_at` strictly before this stage's `started_at` (equal-millisecond timestamps do not count; there is no insertion-order tie-break), the driver cannot insert a planning stage into an already-running pipeline: call `mcp__pp_harness__finalize_stage(stage_id, status="surfaced")` and BREAK, printing the blocker `message`. This gate's predicate is inverted relative to the TDD gate above — a MISSING prior passed planning stage is the violation, not the absence of a gate. Runs with `mode="best_of"` (e.g. `/pp:best-of`, which opens only a `code` stage) are exempt from this gate regardless of taxonomy scope.
