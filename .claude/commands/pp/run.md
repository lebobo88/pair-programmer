...
6. **Stage loop.** Pick the stage set by triage class:
   ...
   - On `outcome="pass"`: call `mcp__pp_harness__get_stage_finalize_readiness(stage_id)` **before** any `finalize_stage(status="passed")`.
     ...
     - If readiness returns `next_action="pass_planning_stage"` (X1b plan-first structural gate — the run's taxonomy scope is `standard`/`major` and no earlier stage with kind `spec`/`repro`/`invariants`/`one_pager`/`gdd`/`mechanic_spec`/`tech_design_doc` has finalized `passed`), the driver cannot insert a planning stage into an already-running pipeline: call `mcp__pp_harness__finalize_stage(stage_id, status="surfaced")` and BREAK, printing the blocker `message`. This gate's predicate is inverted relative to the TDD gate above — a MISSING prior passed planning stage is the violation, not the absence of a gate.
