...
5. **Stage loop.** For each stage in dependency order (default by triage class — see `artifact-conventions.md`):
   ...
   - After any judge `pass` (initial or retry), call `mcp__pp_harness__get_stage_finalize_readiness(stage_id)` **before** attempting `finalize_stage(status="passed")`.
     ...
     - If readiness returns `next_action="pass_planning_stage"` (X1b plan-first structural gate — this `code` stage's run is scope `standard`/`major` and no earlier stage with kind in `spec`/`repro`/`invariants`/`one_pager`/`gdd`/`mechanic_spec`/`tech_design_doc` has finalized `passed` with `finished_at` strictly before this stage's `started_at` (equal-millisecond timestamps do not count; there is no insertion-order tie-break), the driver cannot retroactively insert a planning stage into an already-running pipeline: `finalize_stage(status="surfaced")` and BREAK, printing the blocker `message` so the operator sees the missing planning stage. This gate is inverted relative to the TDD gate above — absence of a prior *passed* planning stage is itself the violation, not an opt-out signal. Runs with `mode="best_of"` (e.g. `/pp:best-of`, which opens only a `code` stage) are exempt from this gate regardless of taxonomy scope.
