...
- `design-system-curator.md` step 2 (after the existing Step 1 Glob for `tokens.json`/`theme/*.ts`/`tailwind.config.*`/`styled-system.config.*`): if the Glob finds a real source, run this skill before composing `design_tokens`/`component_specs`.
