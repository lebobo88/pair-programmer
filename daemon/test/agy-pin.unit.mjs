...
//
// Covers:
//   - DEFAULT_MODELS.agy_* are re-pinned to gemini-3.7-flash-medium.
...
// Verbatim `agy models` output (agy 1.1.23, 2026-09-01).
...
test("E2-1: agy pins point at a served model id, not gemini-3.1-pro-preview", () => {
  assert.equal(DEFAULT_MODELS.agy_critique, "gemini-3.7-flash-medium");
  assert.equal(DEFAULT_MODELS.agy_generate, "gemini-3.7-flash-medium");
...
test("evaluateAgyPin: pin present in served list -> served, no note", () => {
  const res = evaluateAgyPin("gemini-3.7-flash-medium", parseAgyModels(AGY_MODELS_STDOUT));
  assert.equal(res.agy_pin_served, true);
  assert.equal(res.note, null);
  assert.equal(res.pinned_model, "gemini-3.7-flash-medium");
});
...
test("evaluateAgyPin: pin absent from served list -> degraded with a warning", () => {
  ...
  assert.ok(res.served_models.includes("gemini-3.7-flash-medium"));
});

test("evaluateAgyPin: probe failure is fail-soft null, never false", () => {
  const res = evaluateAgyPin("gemini-3.7-flash-medium", null, "`agy models` failed: ENOENT");
...
test("evaluateAgyPin: empty parsed list is inconclusive, not degraded", () => {
  ...
  const res = evaluateAgyPin("gemini-3.7-flash-medium", []);
  assert.equal(res.agy_pin_served, null);
  assert.match(res.note, /inconclusive/);
});
...
test("doctor degradation rule: only a hard false marks google degraded", () => {
  ...
  assert.equal(degradedFor("gemini-3.7-flash-medium"), false);
  assert.equal(evaluateAgyPin("gemini-3.7-flash-medium", null).agy_pin_served === false, false);
});
