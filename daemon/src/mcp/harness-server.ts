...
const TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      ...
      "X1b plan-first gate (code stages only): for a run whose recorded taxonomy mapping scope is 'standard' or 'major', a `code` stage cannot pass unless an earlier stage of a planning kind (spec/repro/invariants/one_pager/gdd/mechanic_spec/tech_design_doc) has already finalized 'passed' with finished_at <= this stage's started_at; surfaces as blocker.gate='plan_first' with next_action='pass_planning_stage'. " +
